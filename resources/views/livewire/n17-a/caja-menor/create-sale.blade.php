                        ...
                        <div class="container px-4">
                            <div class="grid grid-cols-12 gap-2 mb-1">
                                ...
                                <div class="col-span-2 px-6">
                                    <div class="flex items-center justify-center w-full h-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                                        <p class="content-center font-semibold leading-tight text-center text-gray-800 font dark:text-gray-200">MIR</p>
                                    </div>
                                </div>
                                ...
                                <div class="col-span-2">
                                    <div>
                                        ...
                                        <select wire:model.blur="fin_mir" wire:change="GetProposes($event.target.value)" name="fin_mir" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                            <option selected disabled value="">Selecciona una Opción</option>
                                            @foreach ($fines_mir as $fin_mir)
                                                <option value="{{ $fin_mir->NoFin }}">{{ $fin_mir->DescFin }}</option>
                                            @endforeach
                                        </select>
                                        @error('fin_mir') <span class="text-xs text-rose-600">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="mt-4">
                                        <x-label for="proposito_mir" value="{{ __('Proposito *') }}"/>
                                        <select wire:model.blur="proposito_mir" wire:change="GetComponents($event.target.value)" name="proposito_mir" @if(!$mir2) disabled @endif
                                            class="@if(!$mir2) bg-gray-200 @else bg-gray-50 @endif border-gray-300 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">

                                            <option selected disabled value="">Selecciona una Opción</option>
                                            @foreach ($propositos_mir as $proposito_mir)
                                                <option value="{{ $proposito_mir['NoProposito'] }}">{{ $proposito_mir['DescProposito'] }}</option>
                                            @endforeach
                                        </select>
                                        @error('proposito_mir') <span class="text-xs text-rose-600">{{ $message }}</span> @enderror
                                    </div>
                                </div>
                                ...
                                <div class="col-span-2">
                                    <div>
                                        ...
                                        <select wire:model.blur="componente_mir" wire:change="GetActivities($event.target.value)" name="componente_mir" @if(!$mir3) disabled @endif
                                            ...
                                            @foreach ($componetes_mir as $componete_mir)
                                                <option value="{{ $componete_mir['NoComponente'] }}">{{ $componete_mir['DescComponente'] }}</option>
                                            @endforeach
                                        </select>
                                        @error('componente_mir') <span class="text-xs text-rose-600">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="mt-4">
                                        <x-label for="actividad_mir" value="{{ __('Actividad *') }}"/>
                                        <select wire:model.blur="actividad_mir" name="actividad_mir" @if(!$mir4) disabled @endif
                                            ...
                                            @foreach ($actividades_mir as $actividad_mir)
                                                <option value="{{ $actividad_mir['NoActividad'] }}">{{ $actividad_mir['DescActividad'] }}</option>
                                            @endforeach
                                        </select>
                                        @error('actividad_mir') <span class="text-xs text-rose-600">{{ $message }}</span> @enderror
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-12 gap-2 mb-1">
                                ...
                                <div class="col-span-4">
                                    <select wire:model.blur="partida_presupuestal" name="partida_presupuestal"
                                        ...
                                        <option selected disabled value="">Selecciona una Opción</option>
                                        <option value="PTPR-PBR">Partida presupuestal de Prueba</option>
                                    </select>
                                ...
                                <tbody>
                                    @foreach ($elementosCompraMenor as $elemento)
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            ...
                                            <td class="px-6 py-4">
                                                <button type="button" wire:click="RemoveFromList({{ $loop->index }})" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                                                    Eliminar
                                                </button>
                                            </td>
                                        </tr>
                        ...
                        <div class="container px-4">
                            ...
                            <div class="grid grid-cols-12 gap-2 mb-1">
                                ...
                                <div class="col-span-2 px-3 border rounded-lg text-end">
                                    <p class="text-sm font-semibold leading-tight text-gray-800 font dark:text-gray-200">$ {{ $iva }}</p>
                                </div>
                                <div class="col-span-10 text-end">
